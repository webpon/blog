...
- [模块化开发](#模块化开发)
  - [全局样式](#全局样式)
  - [局部css-scoped属性](#局部css-scoped属性)
- [模块化概述](#模块化概述)
  - [**浏览器端模块化规范**](#浏览器端模块化规范)
  - [**服务器端模块化规范**](#服务器端模块化规范)
  - [**大一统的模块化规范-ES6模块化**](#大一统的模块化规范-es6模块化)
- [插件](#插件)
- [vue基本语法](#vue基本语法)
  - [Mustachae语法](#mustachae语法)
  - [v-once](#v-once)
  ...
  - [v-bind](#v-bind)
    ...
    - [绑定内联样式](#绑定内联样式)
    - [添加图片SRC地址](#添加图片src地址)
    - [<component>内置组件 + v-bind: is：实现动态组件](#component内置组件--v-bind-is实现动态组件)
  - [v-for](#v-for)
  - [v-model](#v-model)
  - [v-on](#v-on)
- [数据响应式要注意的点](#数据响应式要注意的点)
- [组件的使用](#组件的使用)
  - [组件全局注册](#组件全局注册)
  - [组件局部注册](#组件局部注册)
- [Vue实例和组件](#vue实例和组件)
  ...
  - [小结](#小结)
- [父子组件通信](#父子组件通信)
  - [通过props向子组件传递数据（传）](#通过props向子组件传递数据传)
  - [通过事件向父组件发送消息（传）](#通过事件向父组件发送消息传)
  - [父组件访问子组件（访）](#父组件访问子组件访)
  - [子组件访问父组件（访）](#子组件访问父组件访)
  - [this.$root](#thisroot)
- [provide/inject](#provideinject)
- [基本使用](#基本使用)
- [`provide/inject` 实现响应式](#provideinject-实现响应式)
- [事件总线（eventBus)](#事件总线eventbus)
- [$attrs和$listeners(常用来封装组件)](#attrs和listeners常用来封装组件)
- [$attrs](#attrs)
- [$listeners](#listeners)
- [slot插槽](#slot插槽)
  - [匿名插槽](#匿名插槽)
  - [具名插槽](#具名插槽)
  - [作用域插槽(vue3.0已废弃)](#作用域插槽vue30已废弃)
  - [$slots](#slots)
- [生命周期](#生命周期)
  - [组件生命周期](#组件生命周期)
  - [父子组件的生命周期](#父子组件的生命周期)
...
- [keep-alive](#keep-alive)
  - [简介](#简介)
  - [keep-alive的应用场景举例](#keep-alive的应用场景举例)
  - [使用方式](#使用方式)
  - [补充](#补充)
- [computed](#computed)
- [watch](#watch)
- [Vue动画](#vue动画)
  ...
  - [CSS 动画](#css-动画)
  - [自定义过渡的类名](#自定义过渡的类名)
  - [在vue中使用animate.css](#在vue中使用animatecss)
  - [同时使用过渡和动画](#同时使用过渡和动画)
  - [显性的过渡持续时间](#显性的过渡持续时间)
- [vue-cli脚手架](#vue-cli脚手架)
  - [环境变量和模式](#环境变量和模式)
  - [简单实现](#简单实现)
- [vue-router路由](#vue-router路由)
  - [Vue**的前进和后退**](#vue的前进和后退)
  - [动态路由](#动态路由)
    - [用法一](#用法一)
    - [用法二](#用法二)
  - [路由传递参数的另外两种方式](#路由传递参数的另外两种方式)
    - [命名路由](#命名路由)
    - [查询参数传递（推荐使用）](#查询参数传递推荐使用)
  - [路由的懒加载](#路由的懒加载)
    - [实现方式](#实现方式)
    - [vue异步组件技术 ==== 异步加载](#vue异步组件技术--异步加载)
    - [组件懒加载方案二 路由懒加载(使用import)](#组件懒加载方案二-路由懒加载使用import)
    - [webpack提供的require.ensure()](#webpack提供的requireensure)
    - [给路由组件文件命名](#给路由组件文件命名)
  - [嵌套路由（子路由）](#嵌套路由子路由)
  - [导航守卫](#导航守卫)
  - [**前端路由如何实现的？**](#前端路由如何实现的)
...
- [vue-x](#vue-x)
  - [State](#state)
    - [**基本使用**（不建议使用）](#基本使用不建议使用)
  - [Mutations](#mutations)
  - [Actions](#actions)
  - [Module](#module)
    - [vuex中模块的基础使用（不开启命名空间时）](#vuex中模块的基础使用不开启命名空间时)
    - [目录结构](#目录结构)
- [vue中使用axios](#vue中使用axios)
  - [基本使用](#基本使用-1)
  - [axios发送并发请求](#axios发送并发请求)
  - [全局配置](#全局配置)
  - [axios的实例](#axios的实例)
  - [axios模块封装](#axios模块封装)
  - [axios拦截器（interceptors)的使用](#axios拦截器interceptors的使用)
  - [处理跨域](#处理跨域)
- [mixins](#mixins)
- [render函数](#render函数)
- [Vue源码解析](#vue源码解析)
  ...
  - [webpack](#webpack)
  - [html -> ast实现`（是基于html的，如果是string则还没了解）`](#html---ast实现是基于html的如果是string则还没了解)
  - [MVVM、MVP和MVC](#mvvmmvp和mvc)
  - [Vue的基本配置](#vue的基本配置)
  - [Vue渲染原理](#vue渲染原理)
    - [1. HTML与模板](#1-html与模板)
    - [2. Vue组件的完整渲染过程](#2-vue组件的完整渲染过程)
  - [模板引擎](#模板引擎)
    - [纯DOM法](#纯dom法)
    - [数组join法](#数组join法)
    - [ES6的反引号法](#es6的反引号法)
  - [Mustache模板引擎](#mustache模板引擎)
    - [基本使用](#基本使用-2)
    - [实现原理](#实现原理)
- [响应式原理](#响应式原理)
  - [如何追踪变化](#如何追踪变化)
